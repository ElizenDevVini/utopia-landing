...
- 2026-07-20 [codex]: Buyer-access diagnosis found 134 of 347 unique valid Sheet wallets still false while the approver was degraded. Live land inventory was 436 open: 52 AAPL and 26 AMZN individually buyable; 193 TSLA and 165 MSFT underfunded. Read-only state-override buys succeeded for AAPL/AMZN and reverted `InsufficientReserve` for TSLA/MSFT.
- 2026-07-20 [codex]: Added a browser read transport that prefers the caching proxy, fails over to the official RPC after three seconds, and bypasses the proxy for one minute after failure. Added exact `maxRewardForSale`-equivalent reserve gating so positive dust cannot create a false buy button.
- 2026-07-20 [codex]: With the proxy deliberately blocked, headless Chrome loaded 591 live plots through fallback. Market suggestions were buyable AAPL plots; TSLA plot 0 and MSFT plot 512 showed no buy button and named AAPL/AMZN as available. Zero console/page exceptions, webhooks, signatures, or transaction methods. JavaScript syntax and diff checks passed; no transaction was broadcast and no funds moved.
